...
router.post("/", auth, async (req: Request, res: Response) => {
  try {
    ...
    const { user, roomname }: { user: string; roomname: string } = req.body;
    ...
    if (adminData) {
      ...
      // console.log("new admin data: ", updatedAdmin);
...
router.get("/getRoomMembers/:roomId", auth, async (req: Request, res: Response) => {
  try {
    const { roomId } = req.params;
    const { user } = req.body;
    const room = await Room.findById(roomId);
    // console.log("getRoomMembers", user._id, roomId);
...
router.get("/getByRoomname/:roomName", auth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (userObj) {
      ...
      Promise.all(userRoomsPromises).then((results) => {
        const searchingRooms = results.filter(
          (room) => room && room.roomname && room.roomname.toLowerCase().includes(roomName.toLowerCase()),
        );
